        ...
        <AuthProvider>
          <Routes>
            <Route element={<ProtectedRoute />}>
              <Route path="/sellers" element={<h1>Sellers fallback</h1>} />
              <Route element={<PermissionRoute module={module} />}>
                <Route path="/admin" element={<h1>Allowed management</h1>} />
              </Route>
            </Route>
          </Routes>
        </AuthProvider>
...
describe('permission-driven administration routes', () => {
  ...
  it.each(['roles_permissions', 'users'])(
    'denies %s:view without the permission tuple',
    async (module) => {
  ...
  it.each(['roles_permissions', 'users'])(
    'allows %s:view with the permission tuple',
    async (module) => {
      renderGate(module, 'user-admin');
      expect(
        await screen.findByRole('heading', { name: 'Allowed management' }),
      ).toBeInTheDocument();
      await waitFor(() =>
        expect(screen.queryByRole('heading', { name: 'Sellers fallback' })).not.toBeInTheDocument(),
      );
    },
  );
});

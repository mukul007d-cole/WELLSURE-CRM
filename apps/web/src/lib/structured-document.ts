import type { CampaignBlock, CampaignDocument, CampaignMark, CampaignSpan } from '../types/domain';

/**
 * Client side of the safe structured-document format.
 *
 * Originally built for the campaign composer
 * (`apps/web/src/pages/admin/campaigns/campaign-document.ts`, which
 * re-exports everything here for backward compatibility) and relocated once
 * the Tools resource library's usage-instructions editor needed the
 * identical model. A `contentEditable` region — cheap, dependency-free, and
 * enough for bold/italic/underline and lists — is walked into this
 * closed-vocabulary document; the server renders the HTML itself, so nothing
 * the browser produced is ever stored verbatim.
 */

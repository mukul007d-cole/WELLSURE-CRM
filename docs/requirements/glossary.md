...
| Status | A single, admin-defined field tracking where a lead sits within its Journey | **Not split into stage+status** — see ADR-0001. Carries `outcome_type` (open/closed_won/closed_lost) and `behavior_type` (default/call_later/follow_up/archived). A Status with an active assignment-routing rule additionally restricts lead visibility (Status Visibility) to the lead's current assignee and that assignee's reporting-hierarchy ancestors — routing decides visibility, not a separately configured Role allow-list — see Phase 20 (originally Phase 19) |

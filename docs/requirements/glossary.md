...
| Status | A single, admin-defined field tracking where a lead sits within its Journey | **Not split into stage+status** — see ADR-0001. Carries `outcome_type` (open/closed_won/closed_lost) and `behavior_type` (default/call_later/follow_up/archived). A Status can additionally carry a Role allow-list governing lead visibility (Status Visibility), independent of `outcome_type`/`behavior_type` — see Phase 19 |

...
| Module | Actions |
|---|---|
| Leads | view, create, edit, comment, delete, export, import, bypass_status_visibility, retain_view_after_reassignment |
...
**E. Status Visibility** — gates who may see a **whole lead** while it sits in a given Status, on every surface (Seller List, search, Seller 360, activity timeline, direct fetch by id) — not field-level redaction, which stays `field_visibility`'s job. Originally (Phase 19) an admin-configured, Role-level allow-list in a `status_visibility` table. **Phase 20 united it with Status Routing**: there is no more allow-list. A Status with no *active routing rule* imposes no restriction at all — the identical "absence means unrestricted" default Phase 19 established, now keyed off routing rather than a table of rows (matching `status_routing_rules`' own "no rule means unrouted"). Once a Status has an active routing rule, visibility narrows to the lead's **current assignee** plus everyone above the assignee in the reporting hierarchy (`users.manager_id`, any depth — the identical relation `TEAM` scope resolves, ADR-0006, asked from the manager's side). No Role plays any part in the check — routing decides visibility for a Role's own `DataScope`, including a Role with `ORGANIZATION` scope. A direct grant (a live Lead Share, or a reassignment-grace grant) is the one exception, covered below — Phase 20 originally made it subject to this narrowing too, and Phase 21 reversed that (ADR-0023).
...
**Every share requires a duration at creation** (Phase 21 Part 1): exactly one of 7, 30, or 60 days, chosen by the person creating it — there is no permanent option. `expiresAt` is computed server-side from the chosen duration; a client can never supply its own expiry timestamp. Expiry is fully passive: a grant simply stops satisfying `getActiveDirectGrant`/the list and count queries once `expires_at` has passed, with no cleanup job — the same mechanism `field_visibility` and every other expiring row in this system already uses.

**Reassignment-grace grants** (Phase 21 Part 2) are the same mechanism used automatically: a user may opt in, on their own account (visible only if their Role holds `leads:retain_view_after_reassignment`), to receive a 30-day `view`-only grant the instant they stop being a lead's assignee — manually or via Status Routing. Created in the same transaction as the reassignment, only when the previous holder is genuinely replaced by a different user. See `docs/architecture/decisions/0023-reassignment-grace-visibility-gating.md`.

...
- The existing lead route also defaults to `defaultAssignmentTypes = ['primary']`. This is a generic synthetic owner convention in code today, but the glossary says assignment types are configurable strings/lookups, not fixed enums. Phase 5 must verify whether the current default is acceptable for API behavior or move assignment-type selection into request/configuration boundaries rather than spreading a hardcoded owner label.
...
4. Lead creation flow:
   ...
   - Evaluate required fields using the target creation Status: plain `required` fields must be present; fields with `requiredFromStatusId` are required only when the initial Status has reached or is at the configured threshold according to the target Journey's configured Status ordering. A later Status threshold must not block creation at an earlier Status.
...
6. Required-from-status handling:
   - Use the configured `statuses.sort_order` within the same Journey to determine whether the current/resulting Status is at or after `required_from_status_id`. Validate same-Journey status references through the configuration data and existing database constraints.
   - If status ordering is ambiguous or duplicate sort orders make threshold semantics unclear, surface a validation/conflict error and document the fixture/test expectation rather than guessing across Journeys.
...
- Assignment owner semantics are underspecified. The schema says `assignment_type` is configurable and the Phase 2 plan warns against hardcoding strings, while `leads.ts` currently uses `defaultAssignmentTypes = ['primary']`. Proposed implementation should centralize the currently established assignment type input and tests should treat it as synthetic fixture configuration, but if Wellsure requires a canonical Lead Owner assignment type, that should be documented before hardcoding it as product behavior.
- `required_from_status_id` threshold semantics depend on Status ordering. The plan proposes using same-Journey `sort_order`, but the docs do not explicitly define “reaches” in terms of ordering. If stakeholders reject that interpretation, a short ADR or requirements clarification is needed before implementation.
...
- Unit tests for `required_from_status_id`: a required-from-later-status Field does not block creation at an earlier Status; the same Field does block creation/editing when the target/current Status is at or after the configured Status threshold; same-Journey Status ordering is used consistently.
